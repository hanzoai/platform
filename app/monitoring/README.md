# Hanzo Monitoring (Go Version)

Application that powers Hanzo's monitoring service.
...
Compatible with all Docker container types (standalone containers, Compose, and Docker Swarm stacks). Note: When monitoring Compose or Swarm stacks, use the `--p` flag to properly identify all services within the stack.
...
Hanzo uses a callback URL to send notifications when metrics exceed configured thresholds. Notifications are sent via POST request in the following format:
