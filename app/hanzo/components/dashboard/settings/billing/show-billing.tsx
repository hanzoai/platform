...
export const ShowBilling = () => {
  ...
  const { mutateAsync: createCustomerPortalSession } = api.stripe.createCustomerPortalSession.useMutation();
      ...
      <Card className="h-full bg-sidebar p-2.5 rounded-xl max-w-full mx-auto">
        <div className="rounded-xl bg-background shadow-md">
          <CardHeader>
            ...
            <CardDescription>Manage your subscription and AI credits</CardDescription>
          </CardHeader>
          <CardContent className="py-8 border-t">
            {isLoading ? (
              ...
              data?.products.map((product) => (
                <section key={product.id} className="border-dashed border-2 p-6 rounded-lg mb-6">
                  ...
                  <div className="my-4">
                    <span>AI Credits (${aiCredits}): ${aiCredits}</span>
                    <Slider
                      min={1}
                      max={500}
                      step={1}
                  ...
                  <div className="my-4">
                    <span>Additional Bases (${additionalBases}): ${(additionalBases * 5).toFixed(2)}</span>
                    <Slider
                      min={0}
                      max={10}
                      step={1}
                  ...
                  <div className="my-4">
                    <span>Additional Apps (${additionalApps}): ${(additionalApps * 5).toFixed(2)}</span>
                    <Slider
                      min={0}
                      max={100}
                      step={1}
                  ...
                  <Button className="mt-4 w-full" onClick={() => handleCheckout(product.id)}>
                    Subscribe
                  </Button>
                </section>
              ))
