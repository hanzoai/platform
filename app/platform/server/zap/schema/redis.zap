# redis.zap — Redis database service capability.
#
# Native ZAP schema replacing the tRPC `redisRouter`
# (server/api/routers/redis.ts). Every method was a `protectedProcedure`
# (authenticated caller whose body enforces per-redis org ownership via
# checkServiceAccess / checkServicePermissionAndAccess and an explicit
# organizationId comparison). `deployWithLogs` was a tRPC `.subscription()` —
# its yielded log lines are collected into an array and returned via the shared
...
# the generic carriers. Compiled by `zapgen schema/redis.zap -out schema/`.
