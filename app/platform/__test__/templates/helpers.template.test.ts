...
import { processValue } from "@hanzo/platform/templates/processors";
...
describe("helpers functions", () => {
	...
	describe("${domain}", () => {
		it("should generate a random domain", () => {
			...
			expect(
				domain.endsWith(`${mockSchema.serverIp.replaceAll(".", "-")}.traefik.me`),
			).toBeTruthy();
		});
	});
	...
	describe("${jwt}", () => {
		it("should generate a JWT string", () => {
			...
			expect(decodedPayload.iss).toEqual("platform");
		});
		it.each([6, 8, 12, 16, 32])(
