 ...
 *
 * Usage:
 *   OLD_SECRET=<old_secret> NEW_SECRET=<new_secret> npx tsx app/platform/scripts/migrate-auth-secret.ts
 *
 * Both OLD_SECRET and NEW_SECRET are required.
 * Run this BEFORE restarting Hanzo Platform with the new secret.
 */
...
if (!OLD_SECRET || !NEW_SECRET) {
	...
	console.error(
		"   Usage: OLD_SECRET=<old> NEW_SECRET=<new> npx tsx app/platform/scripts/migrate-auth-secret.ts",
	);
	process.exit(1);
}
