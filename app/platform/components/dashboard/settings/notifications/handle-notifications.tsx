...
const notificationBaseSchema = z.object({
	...
	platformRestart: z.boolean().default(false),
		...
		if (notification) {
			if (notification.notificationType === "slack") {
				form.reset({
					...
					platformRestart: notification.platformRestart,
	...
	const onSubmit = async (data: NotificationSchema) => {
		...
		if (data.type === "slack") {
			promise = slackMutation.mutateAsync({
				...
				appDeploy: appDeploy,
				platformRestart: platformRestart,
						...
						<div className="flex flex-col gap-4">
							...
							<div className="grid md:grid-cols-2 gap-4">
								...
								{!isCloud && (
									<FormField
										control={form.control}
										name="platformRestart"
										render={({ field }) => (
											<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm gap-2">
												<div className="space-y-0.5">
													<FormLabel>Hanzo Restart</FormLabel>
													<FormDescription>
														Trigger the action when platform is restarted.
													</FormDescription>
												</div>
												<FormControl>
													<Switch
