...
export const runWebServerBackup = async (backup: BackupSchedule) => {
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "platform-backup-"));
		...
		try {
			...
			if (!containerId) {
				writeStream.write("Hanzo postgres container not found❌\n");
				writeStream.end();
				throw new Error("Hanzo postgres container not found");
			}

			writeStream.write(`Hanzo postgres container ID: ${containerId}\n`);

			const postgresContainerId = containerId.trim();

			// First dump the database inside the container
			const dumpCommand = `docker exec ${postgresContainerId} pg_dump -v -Fc -U platform -d platform -f /tmp/database.sql`;
