...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "platform-restore-"));
		...
		try {
			...
			const { stdout: postgresContainer } = await execAsync(
				`docker ps --filter "name=platform-postgres" --filter "status=running" -q | head -n 1`,
			);

			if (!postgresContainer) {
				throw new Error("Hanzo Postgres container not found");
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U platform postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'platform' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U platform postgres -c "DROP DATABASE IF EXISTS platform;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U platform postgres -c "CREATE DATABASE platform;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U platform -d platform /tmp/database.sql`,
			);
