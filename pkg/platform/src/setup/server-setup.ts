...
import { getHanzoUrl } from "@hanzo/platform/services/admin";
import {
...
import {
	findServerById,
	updateServerById,
} from "@hanzo/platform/services/server";
import {
...
const setupMainDirectory = () => `
	# Check if the /etc/platform directory exists
	if [ -d /etc/platform ]; then
		echo "/etc/platform already exists ✅"
	else
		# Create the /etc/platform directory
		mkdir -p /etc/platform
		chmod 777 /etc/platform

		echo "Directory /etc/platform created ✅"
	fi
`;
...
const createTraefikConfig = () => {
	...
	const command = `
	if [ -f "/etc/platform/traefik/dynamic/acme.json" ]; then
		chmod 600 "/etc/platform/traefik/dynamic/acme.json"
	fi
	if [ -f "/etc/platform/traefik/traefik.yml" ]; then
		echo "Traefik config already exists ✅"
	else
		echo "${config}" > /etc/platform/traefik/traefik.yml
	fi
	`;
...
const createDefaultMiddlewares = () => {
	...
	const command = `
	if [ -f "/etc/platform/traefik/dynamic/middlewares.yml" ]; then
		echo "Middlewares config already exists ✅"
	else
		echo "${config}" > /etc/platform/traefik/dynamic/middlewares.yml
	fi
	`;
	return command;
};
	...
	const command = `
		...
		if docker inspect platform-traefik > /dev/null 2>&1; then
			...
			docker run -d \
				...
				--restart always \
				-v /etc/platform/traefik/traefik.yml:/etc/traefik/traefik.yml \
				-v /etc/platform/traefik/dynamic:/etc/platform/traefik/dynamic \
