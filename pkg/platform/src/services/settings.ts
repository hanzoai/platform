...
export const getUpdateData = async (
	...
	try {
		...
		const currentImageTag = getHanzoImageTag();
...
export const reloadDockerResource = async (
	...
	if (resourceType === "service") {
		if (resourceName === "platform") {
			...
			command = `docker service update --force --image hanzoai/platform:${imageTag} ${resourceName}`;
		} else {
...
export const checkPortInUse = async (
	...
	try {
		const command = `docker ps -a --format '{{.Names}}' | grep -v '^platform-traefik$' | while read name; do docker port "$name" 2>/dev/null | grep -q ':${port}' && echo "$name" && break; done || true`;
