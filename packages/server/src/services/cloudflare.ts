 ...
 *
 * For direct uploads, the request body should be a FormData with the project
 * assets. For Git-connected projects, an empty POST triggers a build from the
 * production branch.
 */
...
// ============================================================================
// Pages Custom Domain Management
// ============================================================================
...
export async function addPagesCustomDomain(
	...
	return cfFetch<CloudflarePagesCustomDomain>(
		apiToken,
...
export async function removePagesCustomDomain(
	...
	await cfFetch<unknown>(
		apiToken,
		`/accounts/${accountId}/pages/projects/${projectName}/domains/${domainId}`,
		{ method: "DELETE" },
	);
}
...
/**
 * List Pages projects using account ID from environment config.
 */
export async function listPagesProjectsForDefaultAccount(): Promise<CloudflarePagesProject[]> {
	const config = getCloudflareConfig();
	return listPagesProjects(config.apiToken, config.accountId);
}
