...
export const restoreWebServerBackup = async (
	...
	try {
		...
		try {
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U hanzo postgres -c "CREATE DATABASE hanzo;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U hanzo -d hanzo /tmp/database.sql`,
			);
